...
function checkInstallPromptEligibility() {
  ...
  window.addEventListener('beforeinstallprompt', (e) => {
    ...
    // Only show prompt if we haven't shown it in the last 7 days
    if (!lastPromptTime || (now - parseInt(lastPromptTime, 10)) > 7 * 24 * 60 * 60 * 1000) {
      // Show install button or prompt after a delay
      setTimeout(() => {
        showInstallPrompt();
      }, 60000); // 60 seconds delay for better engagement
    }
  });
...
// Show installation prompt for modern browsers
