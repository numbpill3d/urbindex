...
function switchView(targetViewId) {
  ...
  if (targetView) {
    ...
    if (targetViewId === 'map-container') {
      ...
      setTimeout(() => {
        if (window.mapModule?.map && window.mapModule.map()) {
          console.log('Invalidating map size after view switch');
          window.mapModule.map().invalidateSize(true);

          // Center on user location if available
          if (window.mapModule.getUserLocation) {
            const userLocation = window.mapModule.getUserLocation();
            if (userLocation?.lat && userLocation.lng) {
              window.mapModule.map().setView([userLocation.lat, userLocation.lng], 14);
            }
          }
        } else {
          console.warn('Map module or map object not available');
          // Try to initialize map if it's not available
          if (window.mapModule?.initMap) {
    ...
    if (mapView) {
      ...
      setTimeout(() => {
        if (window.mapModule?.map && window.mapModule.map()) {
          console.log('Invalidating map size after defaulting to map view');
          window.mapModule.map().invalidateSize(true);
        } else if (window.mapModule?.initMap) {
          console.log('Attempting to initialize map');
          window.mapModule.initMap();
        }
      }, 300); // Increased timeout for better reliability
